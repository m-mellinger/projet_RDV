...
Material duckMaterial(1.0, Vec4f(0.9,  0.1, 0.0, 0.0), Vec3f(0.4, 0.4, 0.1),   10.);
...
bool scene_intersect(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, Vec3f &hit, Vec3f &N, Material &material) {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Intersection avec les faces du canard
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    for (size_t j = 0; j<height; j++) { // actual rendering loop
        for (size_t i = 0; i<width; i++) {
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////
            // affichage rouge bleu (anaglyph)
            Vec3f imgRouge = cast_ray(Vec3f(0.1,0,0), Vec3f(dir_x +30, dir_y, dir_z).normalize(), spheres, lights);
            Vec3f imgBleu = cast_ray(Vec3f(-0.1,0,0), Vec3f(dir_x -30, dir_y, dir_z).normalize(), spheres, lights);
            float moy1 = (imgRouge[0]+imgRouge[1]+imgRouge[2])/3;
            float moy2 = (imgBleu[0]+imgBleu[1]+imgBleu[2])/3;
            imgRouge[0] = moy1;
            imgRouge[1] = 0.;
            imgRouge[2] = 0.;
            imgBleu[0] = 0.;
            imgBleu[1] = 0.;
            imgBleu[2] = moy2;
            framebuffer2[i+j*width] = (imgBleu+imgRouge);
            ////////////////////////////////////////////////////////////////////////////////////////////////////////
        }
    }
    ...
    std::vector<unsigned char> pixmap2((width)*height*3);
    for (size_t i = 0; i < height*width; ++i) {
            Vec3f &c = framebuffer[i];
            Vec3f &c2 = framebuffer2[i];
            ...
            for (size_t j = 0; j<3; j++) {
                ...
                pixmap2[i*3+j] = (unsigned char)(255 * std::max(0.f, std::min(1.f, framebuffer2[i][j])));
            }
        }

    stbi_write_jpg("out.jpg", width, height, 3, pixmap.data(), 100);
    stbi_write_jpg("out_with_anaglyph.jpg", width, height, 3, pixmap2.data(), 100);
}
