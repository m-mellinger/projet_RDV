...
Model::Model(const char *filename) : verts(), faces() {
    ...
    for (size_t i = 0; i < faces.size(); i++) {
            Vec3f vec0_1 = point(vert(i,1)) - point(vert(i,0));
            Vec3f vec0_2 = point(vert(i,2)) - point(vert(i,0));
            normal.push_back(cross(vec0_1, vec0_2).normalize());
        }
